...
import { makeRaceListItem } from '../../../lib/__tests__/fixtures';

describe('HoverCard', () => {
  it('大会名が表示される', () => {
    const race = makeRaceListItem({ name_ja: '長野マラソン2026', date: '2026-04-19' });
  ...
  it('en ロケールで name_en が表示される', () => {
    const race = makeRaceListItem({ name_en: 'Nagano Marathon 2026', date: '2026-04-19' });
    render(<HoverCard race={race} locale="en" position={{ x: 0, y: 0 }} />);
    expect(screen.getByText('Nagano Marathon 2026')).toBeInTheDocument();
  });

  it('開催日が表示される', () => {
    const race = makeRaceListItem({ date: '2026-04-19' });
  ...
  it('公式都市名が表示される', () => {
    const race = makeRaceListItem({ city_ja: '長野市', date: '2026-04-19' });
    render(<HoverCard race={race} locale="ja" position={{ x: 0, y: 0 }} />);
