...
## 2026-07-11 セキュリティ対応（Issue #86）

**P0: 依存パッケージ更新**
- next: 16.1.6 → 16.2.10（Middleware/Proxyバイパス・SSRF・DoS修正）
- drizzle-orm: 0.44.7 → 0.45.2（SQL識別子エスケープ不備修正）
- better-auth: 1.6.2 → 1.6.23（デバイス認可フロー脆弱性修正）
- @opennextjs/cloudflare: 1.17.1 → 1.20.1

**P0: BETTER_AUTH_SECRET fail-hard**
- `src/lib/auth.ts`: 本番（NODE_ENV=production）で BETTER_AUTH_SECRET 未設定時に起動時例外。開発・テスト環境ではワーニングのみ

**P0: middleware → proxy リネーム**
- `src/middleware.ts` → `src/proxy.ts`（Next.js 16.2 で "middleware" ファイル規約が deprecated）

**P1: セキュリティヘッダー追加**
- `public/_headers`: Cloudflare Pages 用ヘッダーファイルを新設。X-Content-Type-Options / X-Frame-Options / Referrer-Policy / Permissions-Policy / HSTS / CSP-Report-Only を全ルートに付与

**P1: お問い合わせフォームのスパム対策**
- `src/lib/contact-actions.ts`: honeypot フィールドチェック・最小経過時間チェック（3秒未満は拒否）・IPレート制限（1時間3件超は拒否）・IP/UA記録を追加
- `src/components/contact/ContactForm.tsx`: honeypot フィールド（`_website`）と `form_loaded_at` hidden フィールドを追加
- `src/lib/db/schema.ts`: contact_submissions に ip_address / user_agent カラムと複合インデックスを追加
- `migrations/0017_perfect_boomer.sql`: 上記スキーマ変更のマイグレーション

**P2: JSON-LD XSS エスケープ**
- `src/app/[locale]/races/[id]/page.tsx`: JSON.stringify 出力の `<` を `\u003c` にエスケープ

**P2: 管理ツールのパストラバーサル修正**
- `tools/admin-server.js`: upload ID のサニタイズ・拡張子ホワイトリスト・IMAGES_DIR 外への書き込み防止

**P2: CI actions SHA ピン留め**
- `.github/workflows/test.yml`: actions/checkout・pnpm/action-setup・actions/setup-node をタグではなく SHA で参照。permissions: contents: read を明示。pnpm audit --prod を非ブロッキングで追加
