...
### 適用前の確認
- 文はすべて `INSERT ... ON CONFLICT(race_id, hub_id) DO UPDATE`（冪等なUPSERT、削除・スキーマ変更なし）。39大会 × 8ハブ = 312件
- 参照する39大会がstg・本番とも全件 `races` に存在することを確認（外部キー違反なし）
- stg に先に適用し、値の妥当性を確認: 欠損・0以下なし、最小10分〜最大862分、平均321分
- 受け入れ条件の4大会を判定ルールに当てはめて確認

| 大会 | データ | 判定 |
|---|---|---|
| 神戸マラソン | 前日受付のみ | 前泊必須（前日受付のみ） |
| 板橋Cityマラソン × 東京 | 53分・スタート9:00 | 7:37出発で間に合う → 前泊不要 |
| 伊豆大島 × 東京 | 555分・前日/当日両方受付 | 前泊推奨（当日朝に着く手段が夜行船のみ。値は妥当） |
| 北海道マラソン × 東京 | 前日受付のみ | 前泊必須（前日受付のみ）※受け入れ条件の文言「前泊推奨（間に合わない）」と理由表示は異なるが結論は同じ |

### 適用
```bash
npx wrangler d1 execute krote-run-stg-db --remote --file=./migrations/seed-travel-times.sql
npx wrangler d1 execute krote-run-db --remote --file=./migrations/seed-travel-times.sql
```
両環境とも312件（39大会・8ハブ）になったことを確認。
